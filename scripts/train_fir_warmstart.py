"""Warm-start FIR training: load released DSpark baseline, swap head, freeze backbone.

Only trains the 2.4M FIR additions (update_proj, gate_proj, residual_proj, pos_residual).
...
Usage:
  ...
  PYTHONPATH=. python train_fir_warmstart.py
"""
import os
import sys
import time
...
LOG_EVERY = 10
CKPT_EVERY = 200
CKPT_DIR = os.path.expanduser("~/checkpoints/deepspec/fir_warmstart")
SEED = 42
...
def main(local_rank):
    ...
    fir.markov_w1.weight.data.copy_(old_head.markov_w1.weight.data)
    fir.markov_w2.weight.data.copy_(old_head.markov_w2.weight.data)
    model.markov_head = fir

    for name, param in model.named_parameters():
        param.requires_grad = False
    trainable_names = []
    for name, param in model.markov_head.named_parameters():
        if any(k in name for k in ["lag_transforms", "lag_gate_proj", "hidden_residual"]):
